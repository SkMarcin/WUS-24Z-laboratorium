...
for component in $components; do
  ...
  if [ "$type" == "db_master" ]; then
    ...
    az vm run-command invoke \
      ...
      --resource-group "$resource_group_name" \
      --scripts "@./backend.sh" \
      --parameters "$port" "$db_ip" "$db_port" "$component"
    ...
    backend_ip_name=$(jq -r ".vms[\"$backend_vm\"].public_ip" "$JSON_FILE")
    backend_ip=$(az network public-ip show --resource-group "$resource_group_name" --name "$backend_ip_name" --query "ipAddress" --output tsv)
