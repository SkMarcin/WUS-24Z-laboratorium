...
ssh -i generated_key.pem Admin123@"$ANSIBLE_IP" << EOF
  ...
  echo -e "backend_ip: $BACKEND_IP \n" >> inventories/config$CONFIG_NR/group_vars/all.yml
  echo -e "frontend_ip: $ANSIBLE_IP" >> inventories/config$CONFIG_NR/group_vars/all.yml
